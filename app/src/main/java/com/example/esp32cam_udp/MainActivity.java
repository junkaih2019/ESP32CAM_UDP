...
public class MainActivity extends AppCompatActivity
{
    ...
    public Bitmap UdpTempBitmap = null;                                 //udp中转使用位图定义
    public Boolean BT_OnOffFlag = false;                                //udp接收线程启停位
    public Boolean IP_CheckFlag = true;                                 //IP检查标志位，只开机检查1次
    public volatile Boolean Thread_UDP_RunningFlag = false;             //允许UDP接收线程运行标志位
    public volatile Boolean Thread_FaceDetect_RunningFlag = false;      //允许人脸检测标志位运行标志位
    public volatile Boolean PermitFaceDetectFlag = false;               //允许人脸识别线程运行标志位
    /**
     * 控件定义
     **/
     ...
     * 192.168.31.166
     **/
    public String UDPClientIP = "192.168.4.3";    //客户端IP
    public int UDPClientPort = 11000;                //客户端端口
    /**
     * UDP服务端IP和端口
     **/
    public String UDPSeverIP = "192.168.4.2";     //服务端IP
    ...
    private Handler myHandler = new Handler(Looper.myLooper())
    {
        public void handleMessage(Message msg)
        {
            ...
            {
                case 0:
                    if ((PermitFaceDetectFlag == false) && (Thread_FaceDetect_RunningFlag == true))
                    {
                        UdpTempBitmap = ((Bitmap) msg.obj);
                        PermitFaceDetectFlag = true;
                    }
                    break;
                case 1:
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        OpenCvLoadInit();                       //OpenCV库加载初始化
        ClassiFierInit();                       //初始化人脸级联分类器，必须先初始化
        ...
        BT_UdpOnOff.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                if (BT_OnOffFlag == false)
                {
                    ...
                    Thread_UDP_RunningFlag = true;                                           //UDP接收标志位修改，线程使能
                    ...
                    Thread_FaceDetect_RunningFlag = true;
                    Runnable My_FaceDetection_Runnable = new FaceDetection_Runnable();       //创建My_FaceDetection_Runnable对象
                    Thread FaceDetectThread = new Thread(My_FaceDetection_Runnable);         //人脸检测线程创建并启动
                ...
                else
                {
                    ...
                    Thread_UDP_RunningFlag = false;                          //UDP接收标志位修改，退出线程使能
                    Thread_FaceDetect_RunningFlag = false;                   //人脸检测标志位修改，退出线程使能
...
//        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
     ...
     * ......................OpenCV库加载初始化......................
     **/
    public void OpenCvLoadInit()
    {
        boolean success = OpenCVLoader.initDebug();
        if (success)
        {
            Log.d("TAG", "OpenCV-LoadOK!!!");
            Toast.makeText(getApplication(), "OpenCV-LoadOK!!!", Toast.LENGTH_SHORT).show();
        }
        else
        {
            Log.d("TAG", "OpenCV-LoadFail!!!");
            Toast.makeText(getApplication(), "OpenCV-LoadFail!!!", Toast.LENGTH_SHORT).show();
            finish(); //结束
        }
    }
    ...
    private void ClassiFierInit()
    {
        try
        {
            InputStream is = getResources().openRawResource(R.raw.lbpcascade_frontalface_improved);  //加载模型
...
//    }
     ...
     **/
    //在线程中处理目标检测任务
    class FaceDetection_Runnable implements Runnable
    {
        @Override
        public void run()        //开启线程接收数据
        {
            Log.d("TAG", "=====Face_Detect线程启动=====");
            try
            {
                /*** 人脸识别矩阵定义***/
                Mat matSrc = new Mat();              //源矩阵定义
                Mat matGray = new Mat();             //灰度阵定义
                Mat matDst = new Mat();              //目标阵定义
                MatOfRect faces = new MatOfRect();   //将识别到的人脸区域设定一个区域

                while (Thread_FaceDetect_RunningFlag == true)
                {
//                    Log.d("TAG", "=====Face_Detect_Running=====");

                    if ((PermitFaceDetectFlag == true))
                    {
                        try
                        {
                            Utils.bitmapToMat(UdpTempBitmap, matSrc); //将BitMap文件转化为矩阵
                            Imgproc.cvtColor(matSrc, matGray, Imgproc.COLOR_BGRA2GRAY); //转换为灰度图

                            classifier.detectMultiScale(matGray, faces, 1.05, 3, 0, new Size(30, 30), new Size()); //检查人脸

                            List<Rect> facesList = faces.toList(); //定义目标列表
                            matSrc.copyTo(matDst); //把原图拷贝到目标中，便于在原图上画框
                            if (facesList.size() > 0)
                            {
                                for (Rect rect : facesList)
                                {
                                    Imgproc.rectangle(matDst, rect.tl(), rect.br(), new Scalar(0, 255, 0, 255), 4);
                                }
                            }
                            Bitmap ReturnBitMap = Bitmap.createBitmap(UdpTempBitmap.getWidth(), UdpTempBitmap.getHeight(), Bitmap.Config.ARGB_8888);
                            Utils.matToBitmap(matDst, ReturnBitMap); //将画完框的矩阵转化为位图
                        ...
                        {
                            PermitFaceDetectFlag = false;                            //修改允许人脸识别线程运行标志位，退出线程使能
                        }
                    }
            ...
            {
//                Log.d("TAG", "Face_Detect_Exception_Error: " + e.getMessage());
                e.printStackTrace();
            }
            finally
            {
                PermitFaceDetectFlag = false;                            //修改允许人脸识别线程运行标志位，退出线程使能
                Thread_FaceDetect_RunningFlag = false;                   //修改允许人脸检测标志位运行标志位，退出线程使能
//                Log.d("TAG", "=====Face_Detect线程终止=====");
            }
    ...
    class UDP_Receive_Runnable implements Runnable
    {
        ...
        public void run()        //开启线程接收数据
        {
            try
            {
                ...
                Log.d("TAG", "接受数据： " + UDPSeverAddressIP + "   " + UDPSeverPort + "   " + "RECEIVE_THREAD_On: " + Thread_UDP_RunningFlag);

                while (Thread_UDP_RunningFlag == true)
                {
...
//                Log.d("TAG", "UDPRec_Exception_Error: " + e.getMessage());
            ...
            finally
            {
                ...
                Thread_UDP_RunningFlag = false;                          //修改允许UDP接收线程运行标志位，退出线程使能
